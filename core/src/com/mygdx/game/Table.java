...
public class Table {
	...
	//verification du rendu du tableau pendant le déroulement d'une partie
	...
	// on verifie le bon fonctionnement lord de l'ajout d'une forme dans un endroit du tableau
	public boolean place(Shape shape){
		if(shape == null)return false;//il faut qu'il y'est une formea placer
		int[] position = computePosition(shape);
		if(position == null)return false;//il faut que la position évalué soit dans l'ensemble "tableau"
